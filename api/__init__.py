...
def token_required(f):
    ...
    def decorator(*args, **kwargs):
        ...
        #print (f"Is this your token :  [{token}] ")
...
def token_auth():
    ...
    if request.method == 'POST':
        ...
        tmp_user = User.query.filter_by(username=request.json['username']).first()
        if not tmp_user:
        ...
        if check_password_hash(tmp_user.pass_hash , request.json['password']):
            ...
            token = jwt.encode({'user_id':tmp_user.id,\
                'exp': datetime.utcnow() + timedelta(hours=1)}, current_app.config['SECRET_KEY'], algorithm='HS256')

            return jsonify({"Error":"Success","token":token.decode('UTF-8')}),200
        else:
    ...
    ## create new user if json data found 
        ...
        try:
            new_user = User(username=json_data['username'],\
                pass_hash=generate_password_hash(json_data['password'], "sha256"),\
...
def update_user(id):
    ...
    if request.method == "PUT":
        ...
        try:    
            ...
            db.session.commit()
        # an error will be thrown if the username is already registred by another user in the db 
        except exc.IntegrityError:
            db.roll_back()
            return jsonify({"Error":"Username taken"}), 400 
        
        return jsonify({"Error":f"Successfully updated user {id} values"}) ,200
...
@api.route('/users/<id>', methods=['PUT','PATCH','DELETE', 'GET'])
@token_required
def user_operations(id):
    ...
    ## for deleting user form data base
    if request.method == "DELETE":
    ...
    ## for partialy updating user info 
    if request.method == "PATCH":
    ...
    # for updating all the user info 
    if request.method == "PUT":
        json_data = request.json 
        for item in json_data:
            if json_data[item] == '':
    ...
    # get user info by id 
    if request.method == "GET":
        user_info  = {"username":this_user.username,\
            "first_name":this_user.first_name,\
            "last_name":this_user.last_name,\
            "last_login":str(this_user.last_login),\
            "is_active":this_user.is_active}

        return jsonify({"error":json.dumps(user_info)}),200
